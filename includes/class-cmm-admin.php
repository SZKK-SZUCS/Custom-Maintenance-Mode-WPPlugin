...
class CMM_Admin {
    ...
    public function register_settings() {
        ...
        add_settings_field( 'allowed_roles', 'Engedélyezett szerepkörök', array( $this, 'allowed_roles_render' ), 'cmm-tab-settings', 'cmm_settings_section' );
        add_settings_field( 'redirect_url', 'Automatikus átirányítás (URL)', array( $this, 'redirect_url_render' ), 'cmm-tab-settings', 'cmm_settings_section' );
    ...
    public function allowed_roles_render() {
        $options = get_option( 'cmm_settings' );
        // Ha nincs még beállítva (pl. frissítés előttről), alapértelmezetten admin
        ...
        foreach ( $roles as $role_key => $role_name ) {
            $checked = in_array( $role_key, $allowed_roles, true ) ? 'checked="checked"' : '';
            echo '<label style="display:block; margin-bottom:8px;">';
            echo '<input type="checkbox" name="cmm_settings[allowed_roles][]" value="' . esc_attr( $role_key ) . '" ' . $checked . ' /> ';
            echo esc_html( translate_user_role( $role_name ) );
            echo '</label>';
        }
        echo '</fieldset>';
        echo '<p class="description">Válaszd ki, mely felhasználói szerepkörök láthatják az oldalt karbantartás alatt. Ha üresen hagyod, senki sem fog hozzáférni.</p>';
    }

    public function redirect_url_render() {
        $options = get_option( 'cmm_settings' );
        $redirect_url = isset( $options['redirect_url'] ) ? $options['redirect_url'] : '';
        echo '<input type="url" name="cmm_settings[redirect_url]" value="' . esc_attr( $redirect_url ) . '" class="regular-text" placeholder="https://masik-oldal.hu" style="width: 100%; max-width: 400px;" />';
        echo '<p class="description">Opcionális: Ha megadsz egy URL-t, a jogosultsággal nem rendelkező látogatók a karbantartási képernyő helyett automatikusan ide lesznek átirányítva. <strong>Az adminokat és a fent engedélyezett szerepköröket az átirányítás nem érinti, ők a normál oldalt fogják látni.</strong></p>';
    }
